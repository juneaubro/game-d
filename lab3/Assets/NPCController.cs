...
public class NPCController : MonoBehaviour
{
    ...
    float angularSpeedRadPerSec; // Rad/sec
    ...
    private void HandleAttackingState()
    {
        ...
        {
            float d = Vector3.Distance(this.transform.position, goPlayer.transform.position);
            if(d> CloseEnoughDistance)
            {
                CanSeeAdvesary();//fills bCanSeeAdversary
                if (bCanSeePlayer) {
